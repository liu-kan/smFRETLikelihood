...
def burstBin(full_fname,mat_name):
    ...
    d.calc_bg(fun=bg.exp_fit, time_s=30, tail_min_us='auto', F_bg=5)
    ...
    for burst in bursts:
        avgdd=np.mean(times[mask_dd[burst.istart:burst.istop+1]])*d.clk_p*1e3
        avgda=np.mean(times[mask_ad[burst.istart:burst.istop+1]])*d.clk_p*1e3
        avgaa=np.mean(times[mask_aa[burst.istart:burst.istop+1]])*d.clk_p*1e3
        if abs(avgdd-avgda)>0.5 or abs(avgaa-avgda)>0.5:
            continue
