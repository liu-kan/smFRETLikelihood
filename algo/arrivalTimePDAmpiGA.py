...
def burstBin(full_fname,state,comm,pick,bins,maxiter,logname="pdampilogger.log"):  
    ...
    pdaIns.set_nstates(state, np.random.rand(state).tolist(),[1]*(state-1)*state,[0.1]*state)
    pdaIns.stop=[0]
    fitE=None
    if rank==0:
        fitE=pdaIns.findP()
    else:
        while pdaIns.stop[0]==0:
            # p=copy.deepcopy(pdaIns.rawParams)
            pdaIns.chiSqrArrT(None)    
