...
def appendResult(fn,results,n,timesp,Sth,dbname):
    ...
    fo.write("E:==========="+'\n')
    fo.write(str(genMatE(n,results.x[:n]))+'\n')
    fo.write("K:==========="+'\n')
    k=genMatK(n,results.x[n:n*n])
    fo.write(str(k)+'\n')
    fo.write("P==========="+'\n')
    ...
    def lnLikelihood(self,params,stopp):
        ...
        if rank ==0:
            if (self.minIter-self.oldIter)*self.n_burst>10000:
                ...
                if timesp<1e-100:
                    timesp=1.0
                print("The speed of analysis near %dth Iter is %f burst/s" % (self.minIter,((self.minIter-self.oldIter)*self.n_burst)/timesp))
